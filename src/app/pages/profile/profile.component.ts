...
export class ProfileComponent {
  ...
  submitBankAccount() {
    if (this.bankAccountForm.valid) {
      const bankAccountDetails = this.bankAccountForm.value;

      // API request to send data to the backend
      this.http.post('/api/bank-accounts', bankAccountDetails).subscribe(
        (response) => {
          alert('Bank account added successfully!');
          this.bankAccountForm.reset(); // Reset the form
        },
        (error) => {
  ...
  sendPayout(): void {
    this.http.post('https://localhost:9000/api/payments/payout', 1).subscribe(
      (response: any) => {
        console.log('Payout successful', response);
        alert(`Payout successful: $${response.Amount}`);
      },
      (error) => {
